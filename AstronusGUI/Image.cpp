...
std::map<std::string, ImageData> Image::_imagePool;

Image::Image(const char* filename) {
	programID = LoadShaders("../data/shaders/image.vfs");
...
Image::Image(const char* filename, float x, float y, float subWidth, float subHeight) {
	programID = LoadShaders("../data/shaders/image.vfs");
...
//x/y=position on spritesheet width/height=spritesheet 
void Image::load(float x, float y, float subWidth, float subHeight, float imageWidth, float imageHeight) {
	scale = 1;
...
void Image::render(glm::mat4 mvp) {
...
GLuint Image::loadImage(std::string filename, int &width, int &height, int &channels)
{
	if(_imagePool.find(filename) != _imagePool.end())
	{
		ImageData data = _imagePool.find(filename)->second;
		width = data.width;
		height = data.height;
	...
	_imagePool.insert(std::make_pair(filename, data));
