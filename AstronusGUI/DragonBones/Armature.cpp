...
namespace DragonBones  {
		...
		for(Slot* slot : _slotList)
		{
			...
			if(slot->_isDisplayOnStage)
			{
				...
				if(childArmature)
				{
					...
					//printf("Parent: %s \n", slot->parent->name.c_str());
					//Bone* bone = this->getBone(slot->parent->name);
					//if(bone)
					//{
						//printf("%f %f \n", bone->_globalTransformMatrix[3][0], bone->_globalTransformMatrix[3][1]);
					//}

					//printf("%f %f \n", slot->_globalTransformMatrix[3][0], slot->_globalTransformMatrix[3][1]);
					//printf("%f %f \n", childArmature->getDisplay()->getX(), childArmature->getDisplay()->getY());
				}
			}
		}
	...
	void Armature::addChild(DBObject* object, std::string parentName){
		...
		if(parentName != "")
		{
			...
			if (boneParent)
			{
				//printf("Got boneParent: %s \n", boneParent->name.c_str());
